...
- **Scoped Local Control Plane (preview)** — Attach a host-configured `Principal` with exact or family scopes plus optional exact agent/capability allowlists to the authenticated `RunServer`; health, workflow, agent, approval, interaction, handoff, and event routes authorize before reading request bodies, while identity issuance, TLS, tenancy, federation, and dynamic policy remain host-owned.
