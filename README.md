...
- **Observability** — Full decision traces with bounded provider correlation, optional thread-safe local `TraceSpan`/`SpanRecorder` model-step linkage, configurable stable span sampling, bounded local latency/status metrics with p50/p95/p99 views, agent snapshots, and per-goal token usage tracking with optional hard budgets; hosted exporters and remote trace search remain outside the local contract.
