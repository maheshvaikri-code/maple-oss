...
- **Authenticated Remote Handoff Target (preview)** — Adapt `RunClient.run_agent(...)` into `create_handoff_tool(...)` so a local handoff can send bounded task/context to a host-owned remote agent, bind an explicit local `handoff_id` to the remote run ID, and finalize the local ownership record only after a validated completed result. The adapter supports sync/async callers, fails closed on malformed or incomplete runs, performs no retry, and does not claim in-flight cancellation or exactly-once effects.
