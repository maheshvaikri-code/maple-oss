...
- **Bounded Retrieval Citation Tool (preview)** — `create_retrieval_tool()` adapts an existing lexical backend into a read-only model tool with bounded UTF-8 queries, model-selected top-k limits, deterministic source URI/title citations, metadata-minimal results, whole-output byte bounds, and generic fail-closed backend errors; retrieved text is data only and is never executed. Vector query embedding, corpus authorization, prompt-injection policy, managed stores, and network retrieval remain host-owned.
