...
- **Durable Human Input (preview)** — Durable sync/async ReAct runs can call `request_human_input` to persist a bounded question/form, pause before later tool calls, validate a host response against JSON Schema, reject explicitly, and resume after restart. File-backed input records use per-record cross-process fencing leases; notifications, remote authentication, and multi-round conversations remain outside this contract.
