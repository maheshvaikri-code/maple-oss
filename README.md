...
- **Bounded Event HTTP Ingestion (preview)** — Receive one bounded, authenticated event at a time into a host-owned `EventStream`, preserving its local sequence, timestamp, redaction, and subscriber/exporter behavior; batching, durable replay, fleet aggregation, and remote trace search remain outside the contract.
