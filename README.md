...
- **Bounded Retrieval Citation Tools (preview)** — `create_retrieval_tool()` adapts an existing lexical backend, while `create_vector_retrieval_tool()` composes a host-owned synchronous `EmbeddingProvider` with an existing local vector backend. Both expose read-only model tools with bounded UTF-8 queries, model-selected top-k limits, deterministic source URI/title citations, metadata-minimal results, whole-output byte bounds, and generic fail-closed provider/backend errors; retrieved text and embeddings are data only and are never executed or exposed. Provider/model selection, async embedding, corpus authorization, prompt-injection policy, managed stores, and network retrieval remain host-owned.
