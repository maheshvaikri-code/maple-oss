...
- **Authenticated Remote Task Queue Control (preview)** - `RunServer(task_queue=...)` and `RunClient` expose bounded submit/list/inspect/claim/complete/fail/cancel operations under separate `task:*` scopes, preserving queue ownership and principal agent/capability policy; worker heartbeats, distributed scheduling, automatic retry, handler execution, and exactly-once effects remain outside the contract.
