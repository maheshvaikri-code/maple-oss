...
- **Durable Agent Runs (preview)** — Opt synchronous ReAct goals into bounded in-memory or atomic file checkpoints with stable `run_id` recovery; pending durable approvals pause before further tool side effects and can be resumed after a process restart. Async run-store integration remains a follow-on boundary.
...
Current status: the 101 tracked Python test files report **1,191 passed, 1
skipped in 217.81s**, with no warning output. Focused LLM/autonomy/CLI,
...
network-free doctor reports `ready: true`. The separate workspace Doctrine
