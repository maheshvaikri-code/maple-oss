...
- **Durable Agent Runs (preview)** — Opt synchronous or asynchronous ReAct goals into bounded in-memory or atomic file checkpoints with stable `run_id` recovery; pending durable approvals pause before further tool side effects and can be resumed after a process restart. `FileLeaseManager` provides a standalone cross-process fencing primitive; store ownership, exactly-once effects, and sandboxing remain host-owned or unsupported.
