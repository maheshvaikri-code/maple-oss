"""Tests for ComponentHealthMonitor.snapshot() -- an immediate, on-demand health summary.
...
class TestSnapshot:
    def test_snapshot_is_immediate_and_never_no_data(self):
        m = ComponentHealthMonitor("c")  # NOT started -> no sampled record in history
        snap = m.snapshot()
    ...
    def test_get_health_summary_falls_back_to_on_demand_no_data_gone(self):
        ...
        assert ComponentHealthMonitor("c").get_health_summary()["status"] != "no_data"

    def test_snapshot_status_is_a_valid_live_status(self):
        snap = ComponentHealthMonitor("c").snapshot()
