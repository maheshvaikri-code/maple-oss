...
| 205 | Durable local lexical retrieval | Chief Architect / Backend / Security / QA / Release | docs/briefs/maple-agent-runtime-slice205.md, docs/adr/149-*, docs/plans/maple-agent-runtime-slice205.md, maple/autonomy/retrieval.py, retrieval regressions/exports, API/README/parity/changelog, QA/review evidence | Persist validated source documents and chunking policy in a bounded versioned JSON envelope; rebuild deterministic lexical state after restart; refresh across instances; atomically add/remove under cross-process fencing; fail closed on corrupt state and redacted storage errors; exclude vector persistence, managed stores, network ingestion, distributed coordination, and semantic ranking | complete locally: implementation/docs `b9843d7`, bounded-read hardening `5135877`; focused retrieval `35 passed`; full suite `1838 passed, 1 skipped`; mypy/Black/Ruff/compileall/project pip-audit pass; clean package smoke source `960`, wheel `109`, sdist `874`, build/Twine/install/import/doctor pass; review/QA filed; fresh independent verifier unavailable; publication/cloud/website action remains out of scope |
