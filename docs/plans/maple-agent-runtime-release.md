...
| 157 | Fail-closed pending-request to tool-call correlation | Chief Architect / Backend / Security / QA / Release | `docs/adr/102-*`, sync/async durable agent resume, run regressions, API/README/parity docs, changelog, QA/review evidence | Require the pending approval or human-input record's `tool_call_id` to match a persisted tool-result placeholder before waiting, consuming, replaying, or executing; return `RUN_PENDING_TOOL_MISSING` and preserve request/checkpoint state on mismatch; keep local correlation and at-least-once boundaries explicit | in progress: implementation and focused regressions complete; full static, repository, package, and review evidence pending |
## Slice 127 closure
