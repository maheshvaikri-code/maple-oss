...
| 78 | Explicit unsupported capability inventory | Chief Architect / Security / Release | release brief, README, changelog, QA/review evidence | Cross-reference every remaining `NOT_IMPLEMENTED` runtime path; verify fail-closed tests and no unsupported claim in public feature list | done: documentation-only boundary record; Redis, mutual-TLS, OAuth2, and untrusted-code paths are explicit unsupported/deferred surfaces; no code or dependency change |
