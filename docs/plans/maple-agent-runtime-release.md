...
| 143 | Native async completion for built-in providers | Chief Architect / Backend / ML Engineer / QA / Release | `docs/adr/088-*`, OpenAI/Anthropic providers, provider regressions, API/README/parity docs, changelog, QA/review evidence | Add `complete_async(...)` implementations that await native optional SDK clients for OpenAI-compatible and Anthropic providers; preserve the explicit base-provider synchronous fallback when an async client is unavailable; keep formatting, usage, typed errors, retries, routing, and concurrency semantics bounded | in progress |
## Slice 127 closure
