...
| 155 | Ownership-checked local task failure acknowledgement | Chief Architect / Backend / Security / QA / Release | `docs/adr/100-*`, `TaskQueue.fail_task`, `TaskScheduler.task_failed`, scheduler/failure regressions, API/README/parity docs, changelog, QA/review evidence | Require the recorded agent to fail an `ASSIGNED` or `RUNNING` task through the queue lock with a non-empty error of at most `8,192` UTF-8 bytes; release scheduler assignment/load only after acceptance; reject missing, empty-owner, wrong-owner, oversized-error, and terminal acknowledgements without mutation; keep retry explicit through bounded `requeue_task()` | in progress: implementation `6bf1d6e`; docs/ADR/QA/review and clean-archive evidence pending closure |
## Slice 127 closure
