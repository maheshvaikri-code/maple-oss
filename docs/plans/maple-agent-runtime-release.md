...
| 149 | Bounded global task-queue admission and stale-entry lifecycle | Chief Architect / Backend / Security / QA / Release | `docs/adr/094-*`, `TaskQueue`, task-management regressions, API/README/parity docs, changelog, QA/review evidence | Validate `max_queue_size` as an integer from `1` through `100,000`; enforce one guarded queued-item budget across all priorities; discard cancelled/completed stale tuples before assignment; reject capacity-full requeues before task-state mutation; preserve local in-process semantics with no durable/distributed scheduler claim | done: implementation `8e9077e`; docs/ADR/QA/review `0fe3a45`; focused queue/scheduler suite `42 passed in 0.89s`; full tracked task-management suite `142 passed in 22.28s`; full tracked repository manifest `1375 passed, 1 skipped in 234.42s`; mypy, implementation Black/isort, Ruff, compile, feature-surface diff, clean archive, Twine, install, and task-queue export-smoke gates pass; clean archive wheel `104` entries, sdist `632` entries, workspace-only entries `0`; Bandit unavailable and environment-wide pip-audit remains a veto at `384` vulnerabilities in `77` packages; untracked Doctrine harness aggregate stalled and was interrupted; distributed queues, hosted scheduling, and exactly-once effects remain separate |
## Slice 127 closure
