...
| 167 | Authenticated remote handoff payload delivery | Chief Architect / Backend / Interop / Security / QA / Release | `docs/adr/112-*`, `docs/briefs/maple-agent-runtime-slice167.md`, `RemoteHandoffTarget`, handoff correlation plumbing, server/client/tool regressions, API/README/parity docs, changelog, QA/review evidence | Adapt bounded `RunClient.run_agent(...)` into `create_handoff_tool(...)`; forward allowlisted task/context, bind explicit local `handoff_id` to remote `run_id`, require a validated completed result, preserve local ownership/failure semantics, support sync/async callers, and fail closed without raw remote details; no retries, remote durable restore, scheduling, push delivery, or exactly-once claim | in progress: design `599a9f8`; implementation `aa27498`; focused verification pending |
