...
| 201 | Bounded session history and forking | Chief Architect / Backend / Security / Interop / QA / Release | docs/briefs/maple-agent-runtime-slice201.md, docs/adr/145-*, docs/plans/maple-agent-runtime-slice201.md, maple/autonomy/sessions.py, session regressions, exports, README/API/parity/changelog, QA/review evidence | Retain bounded version-ordered session snapshots in memory and one atomic file envelope; add detached inspection and safe version-based forks with optimistic source checks, legacy direct-snapshot reads, and no execution or message replay | in progress: design `ad969a9`; implementation and evidence pending; no publication/cloud/website action |
