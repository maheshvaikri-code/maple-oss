...
| 161 | Cooperative cancellation through local agent delegation | Chief Architect / Backend / Security / QA / Release | `docs/adr/106-*`, `docs/briefs/maple-agent-runtime-slice161.md`, `Tool`, `create_agent_tool`, `create_handoff_tool`, delegation tests, API/README/parity docs, changelog, QA/review evidence | Pass an exact parent `CancellationToken` to opt-in non-executor handlers and signature-compatible sync/async child targets; reject executor plus handler-injection configuration; preserve legacy targets; finalize cancelled durable handoffs as failed rather than completed; retain cooperative/no-hard-kill and at-least-once boundaries | ready: implementation `ae8855a`; focused delegation/tool suite `49 passed in 0.30s`; full autonomy `490 passed in 20.47s`; exact tracked manifest `1491 passed, 1 skipped in 221.72s`; changed-boundary and whole tracked static/security gates pass; review `docs/reviews/maple-agent-runtime-slice161.md` and QA `docs/qa/maple-agent-runtime-slice161.md` filed; package gate pending; no publication, deployment, cloud action, or website update performed |
