...
| 171 | Bounded remote agent invocation idempotency | Chief Architect / Backend / Interop / Security / QA / Release | `docs/adr/116-*`, `docs/briefs/maple-agent-runtime-slice171.md`, `maple/autonomy/invocations.py`, `maple/autonomy/server.py`, package exports, store/transport regressions, API/README/parity docs, changelog, QA/review evidence | Optional bounded idempotency keys claim before named/capability handler execution, replay detached success/error responses, reject concurrent/conflicting reuse, preserve absent-key compatibility, and provide atomic fenced local restart state without exactly-once claims | in progress: store `d824940`; transport `1c19544`; public docs updated; full regression, review/QA evidence, and clean archive package gate pending |
