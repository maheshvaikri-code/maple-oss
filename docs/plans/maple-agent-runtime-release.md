...
| 111 | Bounded local latency percentile metrics | Chief Architect / Backend / Observability / Security / QA / Release | `docs/adr/057-*`, `maple/autonomy/events.py`, `maple/autonomy/observability.py`, event/observability regressions, API/README/parity docs, changelog, QA/review evidence | Bounded integer sample rings, deterministic p50/p95/p99 metrics for event publish and terminal spans, empty/capacity behavior, preservation of existing counters, static/package/doctor gates | in progress: implementation and focused regressions are being validated; remote aggregation, dashboards, and exporter delivery remain separate |
