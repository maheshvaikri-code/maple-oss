...
| 180 | Least-privilege per-agent and capability policy | Chief Architect / Backend / Security / QA / Release | `docs/briefs/maple-agent-runtime-slice180.md`, `docs/adr/125-*`, `maple/autonomy/server.py`, agent server regressions, README/API/parity/changelog, QA/review evidence | Add bounded exact `Principal` allowlists for agent IDs and capabilities; filter discovery, deny named routes before body parsing, deny capability routes before routing/idempotency, preserve empty-list compatibility, and keep identity federation/tenancy outside scope | complete: implementation `8b97e52`; public docs/review/QA/package gate `abb21c9`; server `52 passed in 22.08s`; full workspace `1722 passed, 1 skipped in 290.87s`; clean archive `847` entries, `1605 passed, 1 skipped` in `237.77s`; wheel `107`, sdist `761`; Twine/install/import/doctor pass; package version remains `1.1.3`; no publication, deployment, cloud action, or website update performed |
