...
| 205 | Durable local lexical retrieval | Chief Architect / Backend / Security / QA / Release | docs/briefs/maple-agent-runtime-slice205.md, docs/adr/149-*, docs/plans/maple-agent-runtime-slice205.md, maple/autonomy/retrieval.py, retrieval regressions/exports, API/README/parity/changelog, QA/review evidence | Persist validated source documents and chunking policy in a bounded versioned JSON envelope; rebuild deterministic lexical state after restart; refresh across instances; atomically add/remove under cross-process fencing; fail closed on corrupt state and redacted storage errors; exclude vector persistence, managed stores, network ingestion, distributed coordination, and semantic ranking | implementation/docs complete locally; focused retrieval `35 passed`; whole-package mypy and changed-file Black/Ruff pass; full suite, clean package smoke, and review/QA evidence pending; publication/cloud/website action remains out of scope |
