...
| 202 | Authenticated local session control plane | Chief Architect / Backend / Interop / Security / QA / Release | docs/briefs/maple-agent-runtime-slice202.md, docs/adr/146-*, docs/plans/maple-agent-runtime-slice202.md, maple/autonomy/server.py, maple/autonomy/sessions.py, server/session regressions, API/README/parity/changelog, QA/review evidence | Add authenticated tip/history inspection and explicit-target data-only session forking over the existing loopback control plane; preserve store ownership, strict bounds, 501 optional-capability compatibility, and no remote mutation/replay/exactly-once claim | gated: brief/design `docs/briefs/maple-agent-runtime-slice202.md` / `docs/adr/146-authenticated-local-session-control-plane.md`; implementation awaits human approval for additive public routes/scopes and session-content exposure; no publication/cloud/website action |
