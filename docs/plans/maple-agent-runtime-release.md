...
| 207 | Bounded retrieval and citation tool | Chief Architect / Backend / Security / QA / Release | docs/briefs/maple-agent-runtime-slice207.md, docs/adr/151-*, docs/plans/maple-agent-runtime-slice207.md, maple/autonomy/retrieval.py, retrieval regressions/exports, API/README/parity/changelog, QA/review evidence | Adapt an existing lexical `RetrievalBackend` into a read-only `Tool` with bounded UTF-8 queries and model-selected top-k, deterministic source URI/title citations, metadata-minimal JSON results, whole-output byte bounds, generic backend/result errors, and no execution of retrieved text; exclude vector query embedding, corpus authorization, network retrieval, managed stores, reranking, and hosted coordination | complete locally: design `a24f57e`, implementation `7fa2d8a`; focused `51 passed`; static gates pass; full suite/package/security/review/QA evidence pending; no publication/cloud/website action |
