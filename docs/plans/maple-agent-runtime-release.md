...
| 184 | Bounded deterministic trace scoring | Chief Architect / Backend / Observability / QA / Release | docs/briefs/maple-agent-runtime-slice184.md, docs/adr/129-*, maple/autonomy/evaluation.py, autonomy/root exports, evaluation regressions, API/README/parity docs, changelog, QA/review evidence | Add versioned identifier-free `TraceEvalSpan`/`TraceEvalCase` fixtures and `EvaluationHarness.run_trace(...)`; score bounded span name/status/parent structure deterministically, project native `TraceSpan` without IDs or attributes, and fail closed on malformed/oversized traces; no semantic faithfulness, provider judge, calibration, hosted aggregation, or trace persistence claim | in progress |
