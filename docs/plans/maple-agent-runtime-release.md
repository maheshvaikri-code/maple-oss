...
| 8 | Release hardening | Security / QA / Release Manager | CI, changelog, release artifacts, package metadata | Full test/lint/type/audit/build matrix and clean-tree checklist | in progress: current dirty suite `1810 passed, 1 skipped`, exact clean tracked suite `1693 passed, 1 skipped`, whole-package mypy/format/lint/compile and clean-environment `pip-audit` pass, plus clean package smoke; CI still has `608` Flake8 `E501` findings and `19` header-policy findings, fresh independent verifier sessions are unavailable, and human approval remains required for public session transport, execution isolation, hosted coordination, and publication |
...
| 211 | Asynchronous document ingestion | Chief Architect / Backend / Interop / Security / QA / Release | docs/briefs/maple-agent-runtime-slice211.md, docs/adr/155-*, docs/plans/maple-agent-runtime-slice211.md, `maple/autonomy/retrieval.py`, retrieval regressions/exports, API/README/parity/changelog, QA/review evidence | Add an async connector and bounded async ingestion helper under the approved public contract and executor/cancellation policy; preserve existing cursor, validation, checkpoint, rate-limit, generic-error, and at-least-once semantics; exclude async index backends, network/provider adapters, corpus policy, remote transport, and exactly-once effects | complete locally 2026-08-29: G0-G5 approved/passed; focused `95 passed`; full `1903 passed, 1 skipped`; static/audit/package smoke pass; Bandit/Gitleaks/fresh verifier unavailable; no publication/cloud/website action |
| 212 | Restricted host-owned HTTP transports | Chief Architect / Backend / Security / QA / Release | docs/briefs/maple-agent-runtime-slice212.md, docs/adr/156-*, maple/autonomy/http_transport.py, event/notification/server regressions, API/parity/changelog, QA/review evidence | Centralize dependency-free HTTP(S)-only opening for five host-owned transports; reject URL credentials, cross-origin redirects, and HTTPS downgrade; preserve proxy support, bounds, typed failures, and Python >=3.8 without new dependencies | implementation `ecaa7e7`; focused transport suite `136 passed`; Black/isort/Ruff/mypy/compile/Bandit pass; full/package gates pending exact final candidate; no publication/cloud/website action |
