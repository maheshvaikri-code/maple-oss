...
| 8 | Release hardening | Security / QA / Release Manager | CI, changelog, release artifacts, package metadata | Full test/lint/type/audit/build matrix and clean-tree checklist | in progress: metadata-clean build, isolated dependency gate, wheel smoke, CI preflight, exact full-suite, and whole-package typing gates now pass; independent fresh-session review and environment-wide dependency governance remain open |
...
| 136 | Bounded async evaluation judges | Chief Architect / Backend / Security / QA / Release | `docs/adr/081-*`, `EvaluationHarness.run_async`, `AsyncEvalJudge`, evaluation regressions, API/README/parity docs, changelog, QA/review evidence | Accept sync/awaitable runners and host-owned sync/awaitable judges in deterministic sequential case order; reuse existing output/schema/trajectory validation and redaction, retain bounded actual tool names, and preserve typed per-case failures; no provider selection, retries, calibration, raw persistence, hosted evaluation, or generated-code execution | done: commit `bfd56b5`; focused async evaluation coverage `4 passed, 22 deselected in 4.61s`; full evaluation suite `26 passed in 0.28s`; exact tracked manifest `1359 passed, 1 skipped in 232.58s` across `108` tracked Python test files; whole-package mypy, changed-surface Black/isort/Ruff/compile/diff/source secret/source dangerous-construct/project-audit gates passed; clean archive build exit `0`, wheel `104` entries, sdist `591` entries, Twine checks passed, clean archive no-dependency async evaluation smoke passed; QA and review records filed; fresh independent review and environment-wide dependency governance remain open; provider-owned judge orchestration, calibration, trace scoring, semantic faithfulness, and hosted evaluation remain separate |
| 136 | Bounded async evaluation judges | Chief Architect / Backend / Security / QA / Release | `docs/adr/081-*`, `EvaluationHarness.run_async`, `AsyncEvalJudge`, evaluation regressions, API/README/parity docs, changelog, QA/review evidence | Accept sync/awaitable runners and host-owned sync/awaitable judges in deterministic sequential case order; reuse existing output/schema/trajectory validation and redaction, retain bounded actual tool names, and preserve typed per-case failures; no provider selection, retries, calibration, raw persistence, hosted evaluation, or generated-code execution | in progress: async implementation and focused evaluation coverage pass; full regression, package, QA, and review closure remain |
