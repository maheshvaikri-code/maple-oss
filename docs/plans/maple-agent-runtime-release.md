...
| 135 | Bounded approval trace correlation | Chief Architect / Backend / Observability / Security / QA / Release | `docs/adr/080-*`, `ApprovalRequest`, agent approval boundary, local trace/event regressions, API/README/parity docs, changelog, QA/review evidence | Persist optional bounded `trace_id`/`span_id` with durable approval requests, surface them in pending errors, and include active model-span plus pending approval identity in normal sync/async `tool.completed` metadata; no prompt/result retention or hosted trace claim | in progress: implementation and focused approval/agent coverage pass; full regression, package, QA, and review closure remain |
