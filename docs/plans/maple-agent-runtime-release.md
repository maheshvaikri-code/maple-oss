...
| 187 | Non-blocking remote claim-next capability matching | Architect / Backend / Security / QA / Release | docs/briefs/maple-agent-runtime-slice187.md, docs/adr/132-*, maple/autonomy/server.py, server regressions, API/README/parity docs, changelog, QA/review evidence | Add authenticated `POST /v1/tasks/claim-next` and `RunClient.claim_next_task(...)`; select compatible queued tasks deterministically by priority/creation/task ID, preserve queue-side ownership, return explicit no-work, and exclude blocking polling, liveness, fairness, distributed scheduling, automatic retry, handler execution, and exactly-once effects | in progress: implementation and focused regressions landed; static, full-workspace, review/QA, and clean-archive package evidence pending; no publication/cloud/website action |
