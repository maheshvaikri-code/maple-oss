...
| 191 | Owner-safe authenticated remote task start lifecycle | Architect / Backend / Security / QA / Release | docs/briefs/maple-agent-runtime-slice191.md, docs/adr/136-*, maple/task_management/task_queue.py, maple/task_management/durable_queue.py, maple/autonomy/server.py, queue/server regressions, API/README/parity docs, changelog, QA/review evidence | Add `POST /v1/tasks/{task_id}/start` and `RunClient.start_task(...)` under `task:start`; enforce queue-side atomic owner/state checks for `ASSIGNED` work, record `started_at`, expose running statistics, preserve durable restart behavior, and exclude leases, heartbeats, scheduling, automatic execution, distributed coordination, and exactly-once effects | complete: implementation `08f71e1`; focused server + durable queue `80 passed`; autonomy + task-management `800 passed`; dirty workspace `1756 passed, 1 skipped`; clean archive `894` entries, `1639 passed, 1 skipped`, wheel `108`, sdist `808`, Twine/install/import/doctor pass; review `docs/reviews/maple-agent-runtime-slice191.md`; QA `docs/qa/maple-agent-runtime-slice191.md`; no publication/cloud/website action |
