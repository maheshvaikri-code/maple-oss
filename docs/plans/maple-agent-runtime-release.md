...
| 169 | Native autonomous-agent remote transport adapter | Chief Architect / Backend / Interop / Security / QA / Release | `docs/adr/114-*`, `docs/briefs/maple-agent-runtime-slice169.md`, `maple/autonomy/agent_transport.py`, package exports, adapter/server regressions, API/README/parity docs, changelog, QA/review evidence | Bind a caller-owned `AgentRunStore` to an `AutonomousAgent`-compatible runtime, register sanitized start/resume callbacks, preserve authenticated typed/raw transport contracts, and reject invalid native identities/statuses without checkpoint transfer or distributed exactly-once claims | done: design `bfbeef8`; implementation/exports `e1812f6`/`b3d8f74`; docs; focused `90 passed in 20.12s`; exact tracked `1528 passed, 1 skipped`; review `docs/reviews/maple-agent-runtime-slice169.md`; QA `docs/qa/maple-agent-runtime-slice169.md`; clean archive source `730`, wheel `104`, sdist `705`, build/Twine/install/import pass; no publication, deployment, cloud action, or website update performed |
