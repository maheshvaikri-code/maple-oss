...
| 175 | Authenticated remote durable checkpoint export and restore | Chief Architect / Backend / Interop / Security / QA / Release | `docs/briefs/maple-agent-runtime-slice175.md`, `docs/adr/120-*`, `maple/autonomy/server.py`, `RunServer`/`RunClient`, remote checkpoint regressions, API/README/parity docs, changelog, QA/review evidence | Add additive `agent:restore`-scoped export and restore routes for complete JSON-safe non-terminal `AgentRunCheckpoint` data; enforce route/store identity, bounded parsing, destination CAS versions, terminal rejection, legacy-store compatibility, and metadata-only receipts without handler execution, automatic retries, scheduling, push delivery, or exactly-once effects | in progress: design artifacts and implementation landed; focused transport regressions `5 passed`; broader regression, static/security, review, QA, and clean package evidence pending |
