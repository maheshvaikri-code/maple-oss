...
| 165 | Authenticated remote handoff result delivery | Chief Architect / Backend / Interop / Security / QA / Release | `docs/adr/110-*`, `docs/briefs/maple-agent-runtime-slice165.md`, `RunServer`/`RunClient` handoff transport, result-scope regressions, API/README/parity docs, changelog, QA/review evidence | Attach a bounded JSON result on authenticated handoff completion and retrieve it through an explicit `handoff:result` scope; keep ordinary inspection redacted, preserve store ownership/validation/fencing, reject invalid payloads without mutation, and do not claim push retries or exactly-once effects | in progress: requirements/ADR `c8abb35`; implementation/docs `3de5e27`; compatibility fix `d346291`; focused `100 passed`; exact tracked `1508 passed, 1 skipped`; review and QA evidence filed; clean package evidence pending |
