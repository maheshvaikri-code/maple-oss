...
|---|-------|------|---------------|-------------------|--------|
| 1 | Add resolver configuration and request principal selection | Backend / Security | maple/autonomy/server.py, exports if needed | Static/resolver exclusivity and per-request selection | todo |
| 2 | Enforce fail-closed resolver and compatibility behavior | Backend / Security / QA | maple/autonomy/server.py, server tests | Generic 401, no body/callback detail, static regression | todo |
| 3 | Public contract, parity, and package evidence | Tech Writer / Release | README/API/parity/changelog, QA/review/release artifacts | Full/static/clean archive/package checks | todo |
...
Done (with evidence): design brief and ADR. Next: implement the bounded
resolver boundary. Blocked on: none.
