...
| Streaming | **Preview** — provider-native OpenAI/Anthropic streams plus bounded sync/async agent lifecycle events with opt-in metadata-only `model.chunk` events, response reconstruction, usage trailers, request correlation IDs, serializable cursors, explicit retention-gap errors, cooperative waiter cancellation, local publish-latency/failure metrics with bounded p50/p95/p99 views, and authenticated single-event/batch ingestion plus cursor reads from a host-owned stream | LangGraph, CrewAI, Microsoft Agent Framework, LlamaIndex, and OpenAI Agents SDK expose run/event streaming | P1: add remote event aggregation, durable replay, and percentile aggregation/backpressure views; local chunk aggregation and remote cursor reads remain deliberately bounded |
...
indexes and make partial item failures explicit.
...
load/append validation. Fleet aggregation and remote trace search remain
separate.
...
3. **Unified streaming and observability:** local provider chunk aggregation
   ...
   chunks, responses, decisions, and normal tool executions. Percentile latency
    views are now local and bounded; add remote event aggregation and durable
    replay while retaining local sampling, cancellation, and the host-owned
    exporter seam. Authenticated bounded ingestion now accepts redacted single
    events or indexed partial-success batches; durable remote replay remains
    separate.
