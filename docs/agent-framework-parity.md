...
| Streaming | **Preview** — provider-native OpenAI/Anthropic streams plus bounded sync/async agent lifecycle events with opt-in metadata-only `model.chunk` events, response reconstruction, usage trailers, request correlation IDs, serializable cursors, explicit retention-gap errors, cooperative waiter cancellation, local publish-latency/failure metrics with bounded p50/p95/p99 views, authenticated single-event/batch ingestion plus cursor reads from a host-owned stream, and an explicit `EventForwarder` with fenced durable consumer cursors for bounded at-least-once remote aggregation | LangGraph, CrewAI, Microsoft Agent Framework, LlamaIndex, and OpenAI Agents SDK expose run/event streaming | P1: add hosted aggregation, automatic scheduling/backpressure, and remote deduplication; local chunk aggregation, remote cursor reads, and explicit forwarding remain deliberately bounded |
...
3. **Unified streaming and observability:** local provider chunk aggregation
   ...
   automatic scheduling/backpressure, and remote deduplication remain separate.
