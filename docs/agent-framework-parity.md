...
| Provider breadth and portable model contracts | **Native abstraction / Partial adapters** — OpenAI, Anthropic, and compatible provider contracts with tool/streaming/structured-output/context/image-input capability routing, deterministic initialization fallback, and opt-in bounded completion failover for classified transient errors | The comparison set has wider provider/integration catalogs and often provider-specific middleware | Add provider-specific contract fixtures; health-aware routing, load balancing, hosted routing, and native-stream failover remain separate; no provider count claims without live contract evidence |
