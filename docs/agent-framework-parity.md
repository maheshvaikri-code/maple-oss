...
| Retrieval, RAG, and citations | **Preview** — bounded host-owned cursor connectors and ingestion, deterministic chunking, local lexical/vector retrieval, source references, provider-neutral bounded host-supplied reranking, retrieval/groundedness evaluation, and optional `InMemoryDocumentCursorCheckpointStore`/`FileDocumentCursorCheckpointStore` restart cursors with atomic revision fencing | LlamaIndex is the strongest reference surface for data connectors, RAG workflows, structured output, and citations; other frameworks expose retrieval integrations | P1: managed-store adapters and connector-specific rate limits; checkpointed sink progress is explicitly at-least-once, and lexical overlap/provider scores are not semantic faithfulness |
