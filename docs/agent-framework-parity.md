...
| Handoffs and agent-as-tool | **Preview** — `create_handoff_tool` is one bounded, synchronous, approval-by-default delegation call with an allowlisted JSON context path for explicit context-aware targets | OpenAI Agents SDK and Microsoft Agent Framework distinguish handoff ownership from manager/agent-as-tool; LlamaIndex documents agent workflows and agents-as-tools | P0: async, durable handoffs with explicit ownership transfer; remote routing remains separate |
