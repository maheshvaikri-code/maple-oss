...
| Tracing and observability | **Preview** — bounded `EventStream`, redaction policy, dependency-free `HttpEventExporter` seam, authenticated single-event/batch ingestion and cursor inspection from a host-owned stream, explicit bounded `EventForwarder` remote aggregation with durable cursors, opt-in `EventForwarderScheduler` polling with bounded local backpressure and lifecycle metrics, decision traces/logger with provider correlation, optional thread-safe local `TraceSpan`/`SpanRecorder` model-step linkage, stable span sampling, local latency/status/failure metrics with bounded p50/p95/p99 views, snapshots | OpenAI Agents SDK tracing and LlamaIndex/CrewAI observability surfaces are broader, especially for exporters and hosted inspection | P1: add hosted aggregation, remote deduplication, hosted scheduling, and approval-replay correlation; local model spans, percentile samples, explicit forwarding, and scheduler metrics remain bounded |
