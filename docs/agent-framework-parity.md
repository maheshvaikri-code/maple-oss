...
| Handoffs and agent-as-tool | **Preview** — `create_handoff_tool` provides one bounded, approval-by-default ownership transfer, while `create_agent_tool` provides manager-style nested invocation that retains caller ownership; both use allowlisted JSON context, explicit sync/async target contracts, bounded target results, and cooperative parent-token propagation to signature-compatible native targets; trusted non-approval agent tools can opt into successful-result replay through the parent execution journal, trusted local handoffs can opt into bounded successful-result replay by explicit `handoff_id`, native child agents can opt into in-flight local resume through explicit `child_run_id` plus `persist_child_run=True`, and `RemoteHandoffTarget` forwards an allowlisted payload through authenticated `RunClient.run_agent(...)` while preserving the local handoff record; capability discovery and deterministic exact-match remote routing are available through the authenticated agent route, with optional host-owned keyed replay | OpenAI Agents SDK and Microsoft Agent Framework distinguish handoff ownership from manager/agent-as-tool; LlamaIndex documents agent workflows and agents-as-tools | P0: remote durable resume, scheduling, push notifications, identity federation, and exactly-once side-effect policy remain separate; bounded remote result pull is available under `handoff:result`, while terminal child-result replay remains governed by the parent journal; keyed invocation replay is local/host-owned and does not make remote handoffs exactly-once |
...
| Durable checkpoints and recovery | **Partial** — JSON-safe memory/file checkpoints, bounded version history, normalized-output journal, opt-in successful tool-result replay including trusted manager-style agent tools, opt-in bounded successful handoff-result replay by explicit ID, opt-in native local in-flight child resume through `persist_child_run=True` and explicit `child_run_id`, built-in approval/input terminal-outcome replay, local run server, sync/async ReAct run resume, native sync/async cooperative cancellation with terminal local state, cooperative cancellation through local delegation, per-run file-backed fencing, approval/input record ownership, authenticated remote inspection/history/resume/cooperative cancellation through explicit host callbacks, bounded synchronous remote handoff payload delivery through `RemoteHandoffTarget`, metadata-only deterministic capability routing, and optional bounded keyed replay for named/capability invocations; history transport is metadata-only and bounded by `agent:read` | LangGraph persistence/time-travel, Microsoft checkpoints/resume, LlamaIndex durable workflows, and CrewAI persisted flows establish a larger durability surface | P0: remote durable restore, distributed scheduling, identity federation, push delivery, and define side-effect policy around remote coordination; bounded capability routing, keyed invocation replay, and remote handoff result pull are available but do not transfer checkpoints, while local child restore is explicit and native-only, terminal child-result replay remains a parent-journal concern, history is inspection-only, provider aborts/hard cancellation, and hosted/distributed coordination require explicit contracts |
