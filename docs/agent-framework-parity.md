...
| Human-in-the-loop approval | **Partial** — durable tool approvals support bounded approve/deny/edit, per-record file-backed cross-process fencing, and the built-in `request_human_input` tool supports one schema-validated response or rejection with sync/async durable resume; `WorkflowPause` supports local pause/resume | LangGraph interrupts, Microsoft request/response ports, LlamaIndex HITL, CrewAI HITL, and OpenAI approval/guardrail patterns cover broader interactive control | P0: add multi-round interaction, host notification/authentication, and apply equivalent ownership to input/run stores; preserve the one-shot boundary until those contracts are specified |
...
| Durable checkpoints and recovery | **Partial** — JSON-safe memory/file checkpoints, bounded history, normalized-output journal, local run server, sync/async ReAct run resume, standalone file-backed fencing, and approval-store record ownership | LangGraph persistence/time-travel, Microsoft checkpoints/resume, LlamaIndex durable workflows, and CrewAI persisted flows establish a larger durability surface | P0: integrate lease ownership into input/run stores, restore broader pending requests, and document side-effect policy |
