...
| Retrieval, RAG, and citations | **Preview** — deterministic chunking, local lexical/vector retrieval, source references, provider-neutral bounded host-supplied reranking, and retrieval/groundedness evaluation | LlamaIndex is the strongest reference surface for data connectors, RAG workflows, structured output, and citations; other frameworks expose retrieval integrations | P1: document connectors and managed-store adapters; do not call lexical overlap or provider scores semantic faithfulness |
