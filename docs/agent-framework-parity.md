...
`resume_handler` enables remote resume through the same normalized result
envelope. Messages and reasoning steps stay off-wire. Scheduling,
cancellation, retries, principal scopes, remote event aggregation, and
exactly-once effects remain host-owned or deferred.
