...
| Durable checkpoints and recovery | **Partial** — JSON-safe memory/file checkpoints, bounded history, normalized-output journal, local run server, sync/async ReAct run resume, and a standalone file-backed fencing lease primitive | LangGraph persistence/time-travel, Microsoft checkpoints/resume, LlamaIndex durable workflows, and CrewAI persisted flows establish a larger durability surface | P0: integrate lease ownership into durable approval/input/run stores, restore broader pending requests, and document side-effect policy |
