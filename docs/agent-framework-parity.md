...
| Handoffs and agent-as-tool | **Preview** — `create_handoff_tool` provides one bounded, approval-by-default ownership transfer, while `create_agent_tool` provides manager-style nested invocation that retains caller ownership; both use allowlisted JSON context, explicit sync/async target contracts, bounded target results, and cooperative parent-token propagation to signature-compatible native targets | OpenAI Agents SDK and Microsoft Agent Framework distinguish handoff ownership from manager/agent-as-tool; LlamaIndex documents agent workflows and agents-as-tools | P0: remote routing/authentication, scheduling, delegated child-run lifecycle/replay, and exactly-once side-effect policy remain separate |
