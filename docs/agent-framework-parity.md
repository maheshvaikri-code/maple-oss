...
| Typed input/output and structured generation | **Preview** — typed tool models, `output_model`, bounded strict finite JSON/schema parsing, typed guardrails, and rejection of non-standard numeric constants or parser recursion failures at the boundary | LlamaIndex, Microsoft Agent Framework, and OpenAI Agents SDK document structured or typed output patterns; the others commonly pair typed state/tools with model schemas | Promote only after broader provider compatibility and failure fixtures |
...
| Retrieval, RAG, and citations | **Preview** — bounded host-owned cursor connectors and ingestion, deterministic chunking, local lexical/vector retrieval, source references, provider-neutral bounded host-supplied reranking, `create_retrieval_tool()` for metadata-minimal read-only lexical search, `create_vector_retrieval_tool()` for one host-owned synchronous `EmbeddingProvider` call, and `create_async_vector_retrieval_tool()` for one awaited host-owned `AsyncEmbeddingProvider` call, each followed by local vector search through a bounded metadata-minimal citation envelope with generic fail-closed errors and no embedding passthrough, plus retrieval/groundedness evaluation, optional `InMemoryDocumentCursorCheckpointStore`/`FileDocumentCursorCheckpointStore` restart cursors with atomic revision fencing, and optional `InMemoryDocumentConnectorRateLimiter` admission control | LlamaIndex is the strongest reference surface for data connectors, RAG workflows, structured output, and citations; other frameworks expose retrieval integrations | P1: managed-store adapters, provider-specific routing, and corpus policy remain host-owned; async local embedding is explicit and executor-backed vector search remains local, while rate limiting is local, fail-fast, and host-owned, checkpointed sink progress is explicitly at-least-once, and lexical overlap/provider scores are not semantic faithfulness |
