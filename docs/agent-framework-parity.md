...
| Guardrails and policy enforcement | **Preview** — input/output guardrails and fail-closed approval with bounded `GuardrailEvent` lifecycle metadata and local run/span-linked `guardrail.*` events; the authenticated local control plane supports exact/family scopes plus optional exact agent/capability allowlists | OpenAI Agents SDK has first-class input/output/tool guardrails; Microsoft Agent Framework documents middleware/security and approval surfaces | Add remote policy engines, token-to-principal resolution, and mandatory hosted audit only through separate host-owned contracts |
| Human-in-the-loop approval | **Partial** — durable tool approvals and file-backed human-input records support bounded decisions with per-record cross-process fencing, and the built-in `request_human_input` tool supports schema-validated response/rejection with sync/async durable resume; local notifier and fail-closed actor-authorizer hooks plus same-record bounded follow-up rounds with persisted history are available; authenticated `RunServer`/`RunClient` approval control routes now list, inspect, and decide without remote consume/execute; `WorkflowPause` supports local pause/resume | LangGraph interrupts, Microsoft request/response ports, LlamaIndex HITL, CrewAI HITL, and OpenAI approval/guardrail patterns cover broader interactive control | P0: add token-to-principal identity, tenancy, and hosted policy; local notifications, exact principal target policy, and bounded-round compatibility are available |
