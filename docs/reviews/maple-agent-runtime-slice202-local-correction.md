# Code Review — Slice 202 local fork-target correction @ b6ef016
...
|---|---|---|---|---|---|
| — | — | `maple/autonomy/sessions.py:1201,1521` | Explicit empty fork IDs were previously treated as omitted IDs and silently replaced with generated IDs. The correction preserves `None` generation while routing explicit values through the existing identifier validator. | — | clean at `b6ef016` |

## Scope check

The diff changes only the target-ID resolution in the built-in in-memory and
file-backed stores and adds one regression covering both implementations. It
does not add routes, scopes, dependencies, storage formats, message replay,
execution, or external actions. The focused test proves an invalid call does
not consume the configured session capacity because a subsequent explicit
fork succeeds.
