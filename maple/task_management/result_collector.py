...
class ResultCollector:
        ...
        with self._lock:
            ...
            if task.created_at:
                ...
                if self._collection_times:
                    self.stats["average_collection_time"] = sum(
                        self._collection_times
                    ) / len(self._collection_times)
    ...
    def _add_result_to_group(self, group: AggregationGroup, result: TaskResult) -> None:
        ...
        if group.aggregation_type == AggregationType.FIRST_COMPLETE:
            ...
            # For other types, we might complete early based on confidence or wait for
            # all
            should_complete = len(group.collected_results) >= group.expected_results
    ...
    def _best_score_aggregation(self, group: AggregationGroup) -> Result[Any, str]:
        ...
        for result in group.collected_results:
            ...
            if group.score_key in result.metadata:
                ...
                isinstance(result.result_data, dict)
                and group.score_key in result.result_data
            ):
    ...
    def add_completion_callback(
        self, group_id: str, callback: Callable[[str, Any], None]
    ) -> Result[None, str]:
    ...
    def get_collection_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            active_groups = len(
                [g for g in self.aggregation_groups.values() if not g.is_complete]
            )
            completed_groups = len(
                [g for g in self.aggregation_groups.values() if g.is_complete]
            )
                ...
                **self.stats,
                ...
                "aggregation_success_rate": (
                    ...
                    / max(
                        1,
                        self.stats["successful_aggregations"]
                        + self.stats["failed_aggregations"],
                    )
                ),
            }
    ...
    def cleanup_old_results(self, max_age_hours: int = 24) -> None:
        ...
        with self._lock:
            ...
            old_group_ids = [
                group_id
                for group_id, group in self.aggregation_groups.items()
                if group.is_complete
                and group.completed_at
                and group.completed_at < cutoff_time
            ]
                ...
                with self._lock:
                    ...
                    for group in self.aggregation_groups.values():
                        if (
                            ...
                            and group.timeout_seconds
                            and current_time - group.created_at > group.timeout_seconds
                        ):
