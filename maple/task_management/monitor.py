...
class TaskMonitor:
    ...
    def start_task_monitoring(
        self, task_id: str, agent_id: str, estimated_duration: Optional[float] = None
    ) -> Result[None, str]:
        ...
        with self._lock:
            for metrics in self.task_metrics.values():
                if metrics.estimated_completion:
                    ...
                    if time_remaining > 0 and time_remaining < (
                        total_time * (1 - self.timeout_warning_threshold)
                    ):
    ...
    def get_monitoring_stats(self) -> MonitoringStats:
        ...
        with self._lock:
            ...
            stats.active_tasks = len(
                [m for m in self.task_metrics.values() if m.progress_percentage < 100]
            )
            ...
            if self._execution_times:
                stats.average_execution_time = sum(self._execution_times) / len(
    ...
    def get_alerts(self, acknowledged: Optional[bool] = None) -> List[TaskAlert]:
        ...
        with self._lock:
            if acknowledged is None:
                ...
                return [
                    alert for alert in self.alerts if alert.acknowledged == acknowledged
                ]
                ...
                for task_id, metrics in tasks_to_check:
                    ...
                    if metrics.estimated_completion:
                        ...
                        if time_remaining <= 0:
                            ...
                            total_time = (
                                metrics.estimated_completion - metrics.start_time
                            )
                            if time_remaining < (
                                total_time * (1 - self.timeout_warning_threshold)
                            ):
