...
class SchedulingPolicy:
    ...
    load_balancing: str = (
        "least_loaded"  # least_loaded, round_robin, capability_weighted
    )
    capability_matching: str = "best_match"  # best_match, first_match, weighted_score
    retry_strategy: str = (
        "exponential_backoff"  # exponential_backoff, linear, immediate
    )
...
class TaskScheduler:
    ...
    def _find_best_agent(self, task: Task) -> Result[str, str]:
        ...
        if self.policy.capability_matching == "first_match":
            return self._find_first_matching_agent(
                capability_requirements, available_agents
            )
        elif self.policy.capability_matching == "best_match":
            return self._find_best_matching_agent(
                capability_requirements, available_agents
            )
        elif self.policy.capability_matching == "weighted_score":
            return self._find_weighted_score_agent(
                capability_requirements, available_agents, task
            )
        else:
            return Result.err(
                f"Unknown capability matching strategy: {self.policy.capability_matching}"
            )

    def _find_first_matching_agent(
        self, requirements: List[CapabilityRequirement], agents: List[AgentInfo]
    ) -> Result[str, str]:
        ...
        matches_result = self.capability_matcher.match_capabilities(
            requirements, agents
        )
    ...
    def _find_weighted_score_agent(
        ...
        with self._lock:
            for match in matches:
                ...
                load_factor = 1.0 - (
                    current_load / self.policy.max_concurrent_per_agent
                )
                ...
                weighted_score = (
                    capability_score * 0.6 + load_factor * 0.3 + priority_factor * 0.1
                )
    ...
    def _apply_load_balancing(self, agents: List[AgentInfo]) -> Result[str, str]:
        ...
        if self.policy.load_balancing == "least_loaded":
            ...
            return Result.err(
                f"Unknown load balancing strategy: {self.policy.load_balancing}"
            )
    ...
    def _find_capability_weighted_agent(
    ...
    def get_scheduling_metrics(self) -> SchedulingMetrics:
        ...
        with self._lock:
            ...
            self._metrics.queue_length = len(
                self.task_queue.list_tasks(TaskStatus.QUEUED)
            )
            ...
            if self._scheduling_times:
                self._metrics.average_scheduling_time = sum(
                    self._scheduling_times
                ) / len(self._scheduling_times)
    ...
    def _scheduler_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                task_result = self.task_queue.get_next_task(
