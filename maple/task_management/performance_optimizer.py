...
from .scheduler import SchedulingPolicy, TaskScheduler
...
class PerformanceOptimizer:
    ...
    def __init__(
        ...
        self.optimization_callbacks: List[
            Callable[[OptimizationRecommendation], None]
        ] = []
    ...
    def analyze_performance(self) -> OptimizationMetrics:
        ...
        if queue_stats.total_tasks > 0:
            metrics.completion_rate = (
                queue_stats.completed_tasks / queue_stats.total_tasks
            )
        ...
        if response_times:
            metrics.p95_response_time = statistics.quantiles(response_times, n=20)[
                18
            ]  # 95th percentile
    ...
    def generate_recommendations(self) -> List[OptimizationRecommendation]:
        ...
        if latest_metrics.overloaded_agents > 0:
            recommendations.append(
                OptimizationRecommendation(
                    ...
                    priority="high",
                    description=(
                        f"{latest_metrics.overloaded_agents} agents are "
        ...
        if latest_metrics.average_wait_time > self.target_response_time:
            recommendations.append(
                OptimizationRecommendation(
                    ...
                    priority="medium",
                    description=(
                        "High average wait time detected. Adjust task priorities."
                    ),
            ...
            recommendations.append(
                OptimizationRecommendation(
                    ...
                    description=(
                        ...
                        f"{latest_metrics.resource_efficiency:.1%}, below target."
                    ),
        ...
        if latest_metrics.tasks_per_minute > 50 and not self.batching_config.enabled:
            recommendations.append(
                OptimizationRecommendation(
                    strategy=OptimizationStrategy.BATCHING,
                    priority="medium",
                    description=(
                        "High task volume detected. Enable batching for efficiency."
                    ),
        ...
        if (
            cache_hit_potential > self.caching_config.cache_hit_threshold
            and not self.caching_config.enabled
        ):
            recommendations.append(
                OptimizationRecommendation(
                    ...
                    description=(
                        "Detected "
                        f"{cache_hit_potential:.1%} potential cache hit rate."
                    ),
    ...
    def apply_optimization(
        ...
        try:
            if recommendation.strategy == OptimizationStrategy.LOAD_BALANCING:
                ...
                return Result.err(
                    f"Unknown optimization strategy: {recommendation.strategy}"
                )
    ...
    def _apply_priority_adjustment(
        ...
        if parameters.get("boost_critical_tasks"):
            ...
            queued_tasks = self.task_queue.list_tasks(
                status=TaskStatus.QUEUED, limit=100
            )
            boosted = 0
    ...
    def _apply_resource_optimization(
        ...
        target_utilization = parameters.get(
    ...
    def _apply_batching_optimization(
        ...
        return Result.ok(
            f"Enabled batching with size {self.batching_config.batch_size}"
        )
    ...
    def _estimate_cache_hit_potential(self) -> float:
        ...
        duplicate_tasks = sum(
            count - 1 for count in task_signatures.values() if count > 1
        )
    ...
    def _optimizer_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                for recommendation in recommendations:
                    if (
                        recommendation.implementation_cost == "low"
