...
class RetryOptions:
    ...
    max_attempts: int = 3
    backoff: Callable[[int], float] = lambda attempt: 0.1 * (
        2**attempt
    )  # Exponential backoff
...
def retry(func: Callable[[], Result[T, E]], options: RetryOptions) -> Result[T, E]:
    ...
    while attempt < options.max_attempts:
        ...
        if options.retryable_errors is not None:
            error_type = (
                error.get("errorType", "UNKNOWN_ERROR")
        ...
        if attempt >= options.max_attempts:
            logger.debug(
                f"Maximum retry attempts ({options.max_attempts}) reached, giving up"
            )
            return result
        ...
        logger.debug(
            f"Retrying after {delay}s (attempt {attempt}/{options.max_attempts})"
        )
        time.sleep(delay)
...
def exponential_backoff(
    ...
    ``RetryOptions.max_attempts`` can stall a caller that holds a resource
    across
