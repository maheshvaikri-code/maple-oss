                    ...
                    return Result.err(
                        {
                            ...
                            "message": (
                                "Response event set but no response or error found"
                            ),
                        }
                    )
            else:
            ...
            if correlation_id in self.pending_requests:
                ...
                if message.message_type == "ERROR":
                    ...
                    request_info["error"] = {
                        ...
                        "message": message.payload.get(
                            "message", "Remote agent returned an error"
                        ),
    ...
    def cleanup_expired_requests(self) -> int:
        ...
        with self._lock:
            for correlation_id, request_info in self.pending_requests.items():
                if (
                    current_time - request_info["timestamp"]
        ...
        with self._lock:
            ...
            if self.pending_requests:
                oldest_timestamp = min(
                    req["timestamp"] for req in self.pending_requests.values()
                )
