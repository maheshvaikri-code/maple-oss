...
class Stream:
    ...
    def connect(
        ...
        subscription = Message(
            ...
            payload={"stream_name": name, "stream_id": stream.stream_id},
        )

        # Send the subscription to all agents
        # This is a simplified approach; in a real system, you'd use a discovery
        # mechanism
    ...
    def send(self, data: Any) -> Result[None, Dict[str, Any]]:
            ...
            return Result.err(
                {
                    ...
                    "details": {"stream_name": self.name, "stream_id": self.stream_id},
                }
            )
        ...
        for subscriber in self.subscribers:
            ...
            if result.is_err():
                logger.warning(
                    f"Failed to send to subscriber {subscriber}: {result.unwrap_err()}"
                )
    ...
    def close(self) -> Result[None, Dict[str, Any]]:
        ...
        close_message = Message(
            ...
            priority=Priority.MEDIUM,
            payload={"stream_name": self.name, "stream_id": self.stream_id},
        )
