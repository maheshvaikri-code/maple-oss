...
class AutoGenAdapter:
    ...
    def create_maple_group_chat(
        self, agents: List[autogen.Agent]
    ) -> "MAPLEEnhancedGroupChat":
        """
        ...
        try:
            ...
            if result.is_ok():
                ...
                self.maple_performance_metrics["average_response_time"] = (
                    ...
                    * (self.maple_performance_metrics["messages_processed"] - 1)
                    + processing_time
                ) / self.maple_performance_metrics["messages_processed"]
            else:
            ...
            self._handle_maple_error(
                {"errorType": "AUTOGEN_SEND_ERROR", "message": str(e)}
            )

    def _convert_to_maple_message(
        self, message: Union[Dict, str], recipient: autogen.Agent
    ) -> Message:
        """
...
class MAPLEEnhancedGroupChat(autogen.GroupChat):
    ...
    def select_speaker(
        self, last_speaker: autogen.Agent, selector: autogen.Agent
    ) -> autogen.Agent:
        """
        ...
        if self.performance_mode == "high_speed":
            ...
            best_agent = min(
                self.agents,
                key=lambda agent: getattr(agent, "maple_performance_metrics", {}).get(
