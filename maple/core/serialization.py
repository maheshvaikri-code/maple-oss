...
class Serializer:
    ...
    def deserialize(
        self, data: bytes, format: Optional[SerializationFormat] = None
                ...
                return Result.err(
                    {
                        "errorType": "UNSUPPORTED_FORMAT",
                        "message": (
                            "Unsupported deserialization format: " f"{format.value}"
                        ),
                    }
                )
            ...
            return Result.err(
                {
                    ...
                    "message": (
                        "MessagePack library not available. Install with: "
                        "pip install msgpack"
                    ),
                }
            )
    ...
    def _restore_from_json(self, data: Any) -> Any:
        ...
        if isinstance(data, dict):
            if "__tuple__" in data:
                return tuple(
                    self._restore_from_json(item) for item in data["__tuple__"]
                )
                ...
                return {
                    key: self._restore_from_json(value) for key, value in data.items()
                }
        elif isinstance(data, list):
