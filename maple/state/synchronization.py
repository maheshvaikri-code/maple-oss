...
class StateSynchronizer:
    ...
    def force_sync(self) -> Result[None, Dict[str, Any]]:
        ...
        try:
            ...
            return Result.err(
                {"errorType": "SYNC_ERROR", "message": f"Force sync failed: {str(e)}"}
            )
    ...
    def _setup_handlers(self) -> None:
        ...
        def handle_sync_request(message: Message) -> Optional[Message]:
            ...
            try:
                ...
                return Message(
                    message_type="STATE_SYNC_RESPONSE", payload={"changes": changes}
                )
        ...
        for peer_id in peers:
            try:
                sync_message = Message(
                    ...
                    ),  # Send one change at a time for simplicity
                )
            ...
            if event.operation == "set":
                ...
                if delete_result.is_err():
                    logger.error(
                        ...
                        f"{event.key}: {delete_result.unwrap_err()}"
                    )
    ...
    def _resolve_conflict(self, event: SyncEvent) -> bool:
        ...
        with self.vv_lock:
            ...
            current_version = self.version_vectors.get(event.source_agent, {}).get(
                event.key, 0
            )
    ...
    def _update_version_vector(self, agent_id: str, key: str, version: int) -> None:
        ...
        with self.vv_lock:
            ...
            self.version_vectors[agent_id][key] = max(
                self.version_vectors[agent_id].get(key, 0), version
            )
    ...
    def _on_state_change(self, key: str, entry: StateEntry) -> None:
        ...
        if self.sync_enabled and self.sync_mode in [
            SyncMode.PUSH,
