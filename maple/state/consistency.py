...
class ConsistencyManager:
    ...
    def __init__(
        ...
        self.causal_history: Dict[str, List[Tuple[str, int]]] = (
            {}
        )  # key -> [(agent, clock)]
    ...
    def get_constraint(self, key: str) -> ConsistencyConstraint:
        ...
        """
        with self.constraints_lock:
            return self.key_constraints.get(
                key, ConsistencyConstraint(model=self.default_model, tolerance_ms=1000)
            )
    ...
    def _strong_read(
        self, key: str, constraint: ConsistencyConstraint
        ...
        if result.is_ok():
            ...
            with self.causality_lock:
                ...
                self.causal_history[key].append(
                    (self.agent.agent_id, self.vector_clock[self.agent.agent_id])
                )
    ...
    def handle_remote_update(
        self, key: str, entry: StateEntry, causal_context: Optional[Dict[str, Any]]
    ) -> None:
        """
            ...
            with self.causality_lock:
                for agent, clock in remote_clock.items():
                    self.vector_clock[agent] = max(
                        self.vector_clock.get(agent, 0), clock
                    )
                ...
                if remote_agent:
                    self.causal_history[key].append(
                        (remote_agent, remote_clock.get(remote_agent, 0))
                    )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self.causality_lock:
            ...
            causal_history_size = sum(
                len(hist) for hist in self.causal_history.values()
            )
