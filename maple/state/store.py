...
class StateStore:
    ...
    def __init__(
        ...
        if backend == StorageBackend.FILE:
            self._file_dir = self.config.get(
                "directory", os.path.join(tempfile.gettempdir(), "maple_state")
            )
        ...
        if backend == StorageBackend.DATABASE:
            self._db_path = self.config.get(
                "database_path", os.path.join(tempfile.gettempdir(), "maple_state.db")
            )
            self._init_database()
    ...
    def delete(
        self, key: str, expected_version: Optional[int] = None
    ...
    def _memory_set(
        self, entry: StateEntry, expected_version: Optional[int]
    ) -> Result[StateEntry, Dict[str, Any]]:
        with self._memory_lock:
    ...
    def _memory_delete(
        self, key: str, expected_version: Optional[int]
    ) -> Result[bool, Dict[str, Any]]:
        with self._memory_lock:
            entry = self._memory_store.get(key)
            if not entry:
    ...
    def _database_set(
        ...
        with self._db_lock:
            try:
                ...
                if expected_version is not None:
                    cursor = conn.execute(
                        "SELECT version FROM state_entries WHERE key = ?", (entry.key,)
                    )
                    row = cursor.fetchone()
    ...
    def _database_delete(
        ...
        with self._db_lock:
            try:
                ...
                if expected_version is not None:
                    cursor = conn.execute(
                        "SELECT version FROM state_entries WHERE key = ?", (key,)
                    )
                    row = cursor.fetchone()
                    if row is None:
                        conn.close()
