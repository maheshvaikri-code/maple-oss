...
class MessageRouter:
                ...
                return Result.err(
                    {
                        ...
                        "message": (
                            "No route found for destination: " f"{message.receiver}"
                        ),
    ...
    def _find_matching_routes(self, destination: str) -> List[RouteEntry]:
        ...
        with self.route_lock:
            ...
            matching.sort(
                key=lambda r: (
                    0
                    if r.pattern == destination
                    else 1 if not r.pattern.endswith("*") else 2
                )
            )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self.health_lock:
            ...
            healthy_count = sum(
                1 for h in self.agent_health.values() if h.get("healthy", True)
            )

        with self.stats_lock:
            total_messages = sum(
                s.get("messages_routed", 0) for s in self.performance_stats.values()
            )
