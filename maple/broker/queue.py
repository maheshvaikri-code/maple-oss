...
class MessageQueue:
    ...
    def __init__(
        ...
        logger.info(
            f"MessageQueue initialized: {queue_type.value}, max_size={max_size}"
        )
    ...
    def enqueue(
        ...
        try:
            ...
            logger.debug(
                f"Message queued: {message.message_id} with priority {priority_value}"
            )
        ...
        while True:
            try:
                ...
                with self._stats_lock:
                    ...
                    self._queue_stats["average_wait_time"] = (
                        current_avg * (count - 1) + wait_time
                    ) / count
            ...
            stats.update(
                {
                    ...
                    "utilization": (
                        len(self._priority_queue) / self.max_size
                        if self.max_size > 0
                        else 0
                    ),
                }
