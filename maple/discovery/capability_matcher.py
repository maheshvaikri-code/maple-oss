...
class CapabilityMatcher:
    ...
    def _evaluate_agent_match(
        self, requirements: List[CapabilityRequirement], agent: AgentInfo
        ...
        for req in requirements:
            ...
            has_capability = self._check_capability_match(
                req.capability, agent.capabilities
            )
        ...
        load_penalty = min(
            agent.load / 0.8, 1.0
        )  # Penalty increases as load approaches 80%
    ...
    def _check_capability_match(
        self, required_capability: str, agent_capabilities: List[str]
    ) -> bool:
    ...
    def suggest_capabilities(
        self, partial: str, available_capabilities: Set[str]
    ) -> List[str]:
    ...
    def get_capability_compatibility_matrix(
        self, agents: List[AgentInfo]
    ) -> Dict[str, Dict[str, bool]]:
        """Generate a compatibility matrix showing which agents have which
        capabilities."""
