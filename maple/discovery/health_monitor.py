...
class HealthMonitor:
    ...
    def _evaluate_health(
        self, agent: AgentInfo, metrics: HealthMetrics
    ) -> Result[HealthStatus, str]:
        ...
        if load_factor > 0.9:
            issues.append(
                f"High task load: {metrics.active_tasks}/{agent.max_concurrent_tasks}"
            )
    ...
    def get_health_trend(
        ...
        with self._lock:
            ...
            trend = [
                metric
                for metric in self.health_history[agent_id]
