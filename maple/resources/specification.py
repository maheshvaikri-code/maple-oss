...
class ResourceRange:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {"min": self.min, "preferred": self.preferred, "max": self.max}

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "ResourceRange":
        """Create from a dictionary."""
        return cls(
            min=data.get("min"), preferred=data.get("preferred"), max=data.get("max")
        )
...
class TimeConstraint:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "TimeConstraint":
        """Create from a dictionary."""
        return cls(deadline=data.get("deadline"), timeout=data.get("timeout"))
...
class ResourceRequest:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.custom:
            result["custom"] = {
                name: rng.to_dict() for name, rng in self.custom.items()
            }
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ResourceRequest":
        ...
        return cls(
            compute=(
                ResourceRange.from_dict(data["compute"]) if "compute" in data else None
            ),
            memory=(
                ResourceRange.from_dict(data["memory"]) if "memory" in data else None
            ),
            bandwidth=(
                ResourceRange.from_dict(data["bandwidth"])
                if "bandwidth" in data
                else None
            ),
            tokens=(
                ResourceRange.from_dict(data["tokens"]) if "tokens" in data else None
            ),
    ...
    @classmethod
    def Range(
        cls, min: Any, preferred: Optional[Any] = None, max: Optional[Any] = None
    ) -> ResourceRange:
    ...
    @classmethod
    def TimeConstraint(
        cls, deadline: Optional[str] = None, timeout: Optional[str] = None
    ) -> TimeConstraint:
