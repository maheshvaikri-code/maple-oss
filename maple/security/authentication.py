...
class AuthenticationManager:
    ...
    def authenticate(
        self, credentials: Union[Dict[str, Any], AuthCredentials]
    ) -> Result[AuthToken, Dict[str, Any]]:
        """
    ...
    def _authenticate_jwt(
        ...
        try:
            ...
            try:
                ...
                return Result.err(
                    {"errorType": "JWT_EXPIRED", "message": "JWT token has expired"}
                )
    ...
    def _authenticate_certificate(
        ...
        try:
            ...
            if certificate_pem in self.trusted_certificates:
                ...
                logger.info(
                    f"Certificate authentication successful for principal: {principal}"
                )
                return Result.ok(auth_token)
            else:
                return Result.err(
    ...
    def _authenticate_api_key(
        ...
        try:
            ...
            if not api_key:
                return Result.err(
                    {"errorType": "MISSING_API_KEY", "message": "API key is required"}
                )
            ...
            if api_key in self.api_keys:
                ...
                if (
                    key_info.get("expires_at") is not None
                    and time.time() > key_info["expires_at"]
                ):
                    return Result.err(
                        {
                ...
                session_token = base64.b64encode(
                    hashlib.sha256(
                        f"{principal}:{api_key}:{time.time()}".encode()
                    ).digest()
                ).decode("utf-8")
                ...
                logger.info(
                    f"API key authentication successful for principal: {principal}"
                )
                return Result.ok(auth_token)
            else:
                return Result.err(
                    {"errorType": "INVALID_API_KEY", "message": "API key is not valid"}
                )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        expired_count = sum(
            1 for token in self.active_tokens.values() if not token.is_valid()
        )
