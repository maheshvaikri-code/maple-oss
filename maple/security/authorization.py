...
class AccessPolicy:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "denied_principals": (
                list(self.denied_principals) if self.denied_principals else None
            ),
        }
...
class AuthorizationManager:
    ...
    def create_role(
        ...
        try:
            ...
            role = Role(
                name=name, permissions=set(permissions), description=description
            )
    ...
    def authorize(
        ...
        try:
            ...
            if (
                required_permission in principal_permissions
                or "admin" in principal_permissions
            ):
                # Check resource-specific policies
                if self._check_resource_policies(
    ...
    def add_policy(self, policy: AccessPolicy) -> None:
        ...
        logger.info(
            f"Added access policy for resource pattern: {policy.resource_pattern}"
        )
    ...
    def remove_policy(self, resource_pattern: str) -> bool:
        ...
        with self.policies_lock:
            ...
            self.policies = [
                p for p in self.policies if p.resource_pattern != resource_pattern
            ]
            removed = len(self.policies) < original_count

        if removed:
            logger.info(
                f"Removed access policies for resource pattern: {resource_pattern}"
            )
    ...
    def _check_resource_policies(
        self, principal: str, permission: str, resource: str
    ) -> bool:
        """Check resource-specific policies."""
        with self.policies_lock:
                ...
                if policy.matches_resource(resource):
                    ...
                    if permission in policy.required_permissions:
                        ...
                        if (
                            ...
                            and principal in policy.denied_principals
                        ):
                            return False
    ...
    def _is_denied(self, principal: str, resource: str) -> bool:
        ...
        with self.policies_lock:
            for policy in self.policies:
                if (
                    policy.matches_resource(resource)
                    and policy.denied_principals
                    and principal in policy.denied_principals
                ):
                    return True
