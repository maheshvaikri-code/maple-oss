...
class AuditLogger:
    ...
    def log_link_failed(self, agent_a: str, agent_b: str, reason: str) -> str:
        ...
        return self.log_event(
            ...
            agent_id=agent_a,
            action="establish_link",
            details={"peer_agent": agent_b, "failure_reason": reason},
        )
    ...
    def log_token_revoked(self, principal: str, token_id: str, reason: str) -> str:
        ...
        return self.log_event(
            ...
            principal=principal,
            action="revoke_token",
            details={"token_id": token_id, "revocation_reason": reason},
        )
    ...
    def get_events(
        ...
        with self.events_lock:
            ...
            if principal:
                filtered_events = [
                    e for e in filtered_events if e.principal == principal
                ]
    ...
    def _print_to_console(self, event: AuditEvent) -> None:
        ...
        print(
            f"{severity_icon} [{timestamp_str}] {event.event_type.value}: {event.result}"
        )
        if event.principal:
...
def get_audit_logger() -> AuditLogger:
    ...
    if _audit_logger is None:
        _audit_logger = AuditLogger(
            log_file="maple_security_audit.log", enable_console=False
        )
